# Dairy R&D Chrome extension

Install on **Chrome** (or Edge). No email and no desktop app.

## What it does

- Choose your department once
- At **4:00 PM IST** each day, Chrome pops: **this week: N · this month: M**
- In the popup, toggle **Today / Yesterday / Week / Month / Year / All time**

If Chrome is closed at 4 PM, the pop happens the next time you open Chrome after 4:00 PM IST that day.

## Install

1. Open Chrome → `chrome://extensions`
2. Turn **Developer mode** on
3. **Load unpacked** → select this `extension` folder
4. Click the puzzle-piece icon → pin **Dairy R&D Department Briefing**
5. Click the teal **RD** icon and pick your department

After you change the files, click **Reload** on `chrome://extensions`.

## Test

Click the extension → **Pop notification now**.
